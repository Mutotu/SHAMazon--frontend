...
function CartPage(props) {
    ...
    function createPlantItems() {
        return (
            cart.map((plant, idx) => {
                return (
                    <div>
                        <PlantItem key={plant.id} name={plant.name} image={plant.image} price={plant.price} />
                        <button onClick={() => { handleRemoveCartClick(idx); }}>Remove from Cart</button>
                    </div>
                )
            })
        )
    ...
    return (
        <div>
            ...
            <button onClick={() => { navigation('/checkout'); }}>Checkout</button>
        </div>
    )
}
